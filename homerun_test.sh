...
# Quick test script to validate all components of the pipeline
...
# train the tokenizer on less data (100M chars is enough for quick test)
...
# pretrain the d4 model for just 10 steps
python -m scripts.base_train --depth=4 --max_seq_len=512 --device_batch_size=4 --run=$WANDB_RUN --model_tag=d4-forward --num_iterations=10
# evaluate the model on a smaller chunk of train/val data
python -m scripts.base_loss --device_batch_size=4 --model_tag=d4-forward
# evaluate the model on CORE tasks (with fewer examples for speed)
python -m scripts.base_eval --max-per-task=20 --model_tag=d4-forward
...
# run midtraining for just 10 steps (reduced batch size due to longer sequences)
python -m scripts.mid_train --device_batch_size=1 --run=$WANDB_RUN --num_iterations=10
python -m scripts.chat_eval -i mid -x 10

# -----------------------------------------------------------------------------
# Supervised Finetuning - 10 steps only

# train sft for just 10 steps (reduced batch size due to longer sequences)
python -m scripts.chat_sft --device_batch_size=1 --run=$WANDB_RUN --num_iterations=10
python -m scripts.chat_eval -i sft -x 10

# -----------------------------------------------------------------------------
# Test chat interface (just verify it loads)
echo ""
echo "Pipeline test complete! All components validated."
...
echo "For actual training, use homerun.sh (1-2 hours) or speedrun.sh (4 hours on 8xH100)."
